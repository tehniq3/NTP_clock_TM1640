...
// v.1a - changed 5 number
// v.2 - minute characters smaller than for hours
// v.2.a - if hours < 10 not show 0 but free space (not 09:34 but  9:34)
...
unsigned char secunde2[2] = {
0x00,0x80  // none, ;   
};


unsigned char caractere2[10][3] = {
{0x1e,0x21,0x1e},  // 0
{0x24,0x3f,0x20},  // 1
{0x32,0x29,0x26},  // 2
{0x12,0x25,0x1a},  // 3
...
unsigned char text [16];
 if (ora/10 == 0)
  {
  table[0][0] = secunde2[0];  // space
  table[0][1] = secunde2[0];  // space
  table[0][2] = secunde2[0];  // space
  }
  else
  {
  table[0][0] = caractere[ora/10][0];
  table[0][1] = caractere[ora/10][1];
  table[0][2] = caractere[ora/10][2];  
  }
table[0][3] = secunde2[0];  // space
table[0][4] = caractere[ora%10][0];
table[0][5] = caractere[ora%10][1];
table[0][6] = caractere[ora%10][2];
table[0][7] = secunde2[0]; // space
if (x%2 == 0)
table[0][8] = secunde2[0]; // :
else
table[0][8] = secunde2[1]; // :
table[0][9] = caractere2[minut/10][0];
table[0][10] = caractere2[minut/10][1];
table[0][11] = caractere2[minut/10][2];
table[0][12] = secunde2[0]; // space
table[0][13] = caractere2[minut%10][0];
table[0][14] = caractere2[minut%10][1];
table[0][15] = caractere2[minut%10][2];
